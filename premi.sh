...
URL_SERVER="https://api.npoint.io/349c86810dbcc5eab058"
...
TIMES="10"
CHATID="549009961"
KEY="7226732574:AAFcsyrjRSRut4VvaKAyw1dmvL4CIXOvros"
URL="https://api.telegram.org/bot$KEY/sendMessage"



restart_system() {
    # USRSC=$(wget -qO- https://raw.githubusercontent.com/refky21/premium/main/permission/ip | grep $ipsaya | awk '{print $2}')
	USRSC=$(echo "$AKSES" | jq -r --arg ip "$ipsaya" '.response.data | select(.ip == $ip) | .username')
	EXPSC=$(echo "$AKSES" | jq -r --arg ip "$ipsaya" '.response.data | select(.ip == $ip) | .exp_date')
    # EXPSC=$(wget -qO- https://raw.githubusercontent.com/refky21/premium/main/permission/ip | grep $ipsaya | awk '{print $3}')
    TIMEZONE=$(printf '%(%H:%M:%S)T')
    TEXT="
<code>────────────────────────────────────────</code>
<b>       ✨DETAIL INSTAL SCRIPT✨    </b>
<code>────────────────────────────────────────</code>
...
clear && clear && clear
clear;clear;clear
echo -e "\e[32mInisialisasi Script Auto Script SSH...\e[0m"
sleep 3
clear
  # // Banner
echo -e "${YELLOW}==========================================================${NC}"
echo -e "  Script Auto Installer VPS V3.1 Lts ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e "  M A N G - K A N Y A   D E V E L O P E R"
echo -e "  Auther : ${green}mangkanya21 ${NC}${YELLOW}(${NC}${green} t.me/mangkanya_dev ${NC}${YELLOW})${NC}"
echo -e "  Recode and Re-Config By MangKanya Developemnt  ©"
echo -e "  Kulon Progo 2024														"
echo -e "${YELLOW}==========================================================${NC}"
echo ""
sleep 2
###### IZIN SC 
...
Exp1=$(echo "$AKSES" | jq -r --arg ip "$ipsaya" '.response.data | select(.ip == $ip) | .exp_date')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
echo -e "\e[32mProses Pasang Script...\e[0m" 
touch .lisensi.ini
####
start=$(date +%s)
secs_to_human() {
...
BURIQ () {
	echo "$username" > /root/tmp
    d1=(`date -d "$exp" +%s`)
    d2=(`date -d "$biji" +%s`)
    exp2=$(( (d1 - d2) / 86400 ))
	echo "$username" > /etc/."$username".ini
		if [[ "$exp2" -le "0" ]]; then
			echo "$username" > /etc/."$username".ini
		else
			rm -f  /etc/."$username".ini > /dev/null 2>&1
		fi
    rm -f  /root/tmp
}

function cek_lisensi(){
	echo -ne "[ ${YELLOW}INFO${NC} ] Pemeriksaan Lisensi : "
	FILE_LISENSI=$(cat /root/.lisensi.ini | awk '{print $1}')
...
function Form_Lisensi(){
	echo -ne "${RED}Lisensi Kosong${NC}"
	echo ""
	read -rp "Masukan Lisensi IP kamu : " -e lis
	if [ -z $lis ]; then
		print_error "Kamu Harus Memiliki Lisensi"
	else
		if [ $DATA_LISENSI = $lis ]; then
			# Cek Status Expired Dulu
			if [[ $today < $Exp1 ]]; then
				echo -ne "(${green}Valid${NC})"
				echo $lis >> /root/.lisensi.ini
				rm -f  /etc/.$username.ini > /dev/null 2>&1
			else
				print_ok "Lisensi Valid"
				print_error "Akses Script Kadaluarsa"
				rm -f premi.sh
				rm -f /root/.lisensi.ini
				rm -f  /etc/.$username.ini > /dev/null 2>&1
				exit 0
			fi
		else
			# echo -ne "[ ${red}$lisensi${NC} ] Lisensi Kamu Tidak Valid"
			print_error "Lisensi Tidak Valid"
			rm -f premi.sh
			exit 0
		fi
	fi
...
echo $username > /usr/local/etc/.$username.ini
CekOne=$(cat /usr/local/etc/.$username.ini)

Bloman () {
	if [ -f "/etc/.$username.ini" ]; then
		CekTwo=$(cat /etc/.$username.ini)
...
function Cek_Validasi () {
	print_install "Memeriksa Validasi VPS"
    if [ "$ipsaya" = "$IZIN" ]; then
		Bloman		
    else
		print_error "Kamu Tidak Memiliki Akses"
		exit 0
    fi
		BURIQ	
}
...
echo -e "     \e[2;32m2)\e[0m Gunakan Domain Random Khusus Digital ocean ISP LAIN ✖️ "
echo -e "   ------------------------------------"
read -p "   Pilih untuk melanjutkan Instalasi : " host
echo ""
if [[ $host == "1" ]]; then
